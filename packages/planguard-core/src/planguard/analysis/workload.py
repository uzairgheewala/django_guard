...
def builtin_motifs(producer: ProducerIdentity) -> tuple[WorkloadMotifArtifact, ...]:
    ...
    for payload in definitions:
        artifact_id = _stable_id("wmotif", {"definition_version": "builtin-workload-motif.v2", "payload": payload.model_dump(mode="python"), "producer": producer.model_dump(mode="python")}, length=32)
        output.append(
            WorkloadMotifArtifact(
                created_at=semantic_epoch(),
                artifact_id=artifact_id,
                producer=producer,
                provenance=Provenance(derivation_key="builtin-workload-motif.v2"),
                payload=payload,
            ).seal()
        )
    return tuple(output)
