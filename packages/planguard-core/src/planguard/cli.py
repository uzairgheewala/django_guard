"""PlanGuard Milestone G command-line interface."""
...
from planguard.artifacts.models import (
    ...
    BenchmarkProtocolArtifact, ExperimentSeriesArtifact, PluginManifestArtifact,
    DemonstrationCaseArtifact, SecurityAuditArtifact, ArtifactTrustReportArtifact,
...
def _parser() -> argparse.ArgumentParser:
    ...
    benchmark_catalog = subcommands.add_parser("benchmark-catalog", help="Persist and list built-in benchmark protocols")
    benchmark_catalog.add_argument("--store", type=Path, default=Path(".planguard"))

    benchmark_run = subcommands.add_parser("benchmark-run", help="Run a deterministic synthetic benchmark series")
    benchmark_run.add_argument("protocol_id")
    benchmark_run.add_argument("--store", type=Path, default=Path(".planguard"))
    benchmark_run.add_argument("--metric-model", choices=("constant", "linear", "superlinear", "threshold"), default="linear")
    benchmark_run.add_argument("--persist", action="store_true")

    security_audit = subcommands.add_parser("security-audit", help="Scan artifacts for integrity and sensitive capture risks")
    security_audit.add_argument("artifact_ids", nargs="*")
    security_audit.add_argument("--store", type=Path, default=Path(".planguard"))
    security_audit.add_argument("--all", action="store_true")
    security_audit.add_argument("--persist", action="store_true")

    sanitize = subcommands.add_parser("sanitize-artifact", help="Create a redacted derivative artifact and receipt")
    sanitize.add_argument("artifact_id")
    sanitize.add_argument("--store", type=Path, default=Path(".planguard"))
    sanitize.add_argument("--persist", action="store_true")

    trust = subcommands.add_parser("trust-verify", help="Verify artifact integrity and references")
    trust.add_argument("artifact_ids", nargs="*")
    trust.add_argument("--store", type=Path, default=Path(".planguard"))
    trust.add_argument("--all", action="store_true")
    trust.add_argument("--persist", action="store_true")

    plugins = subcommands.add_parser("plugin-list", help="List built-in and discovered plugin manifests")
    plugins.add_argument("--store", type=Path, default=Path(".planguard"))
    plugins.add_argument("--discover", action="store_true")
    plugins.add_argument("--persist", action="store_true")

    demos = subcommands.add_parser("demo-verify", help="Verify one or all demonstration cases")
    demos.add_argument("case_ids", nargs="*")
    demos.add_argument("--store", type=Path, default=Path(".planguard"))

    release = subcommands.add_parser("release-build", help="Build a versioned OSS release manifest")
    release.add_argument("--store", type=Path, default=Path(".planguard"))
    release.add_argument("--release-key", default="planguard-0.7.0")
    release.add_argument("--status", choices=("draft", "candidate", "verified"), default="candidate")
    release.add_argument("--persist", action="store_true")
    ...
    try:
        ...
        if args.command == "policy-evaluate":
            ...
            evaluation = evaluate_policy(
                bundle,
                policy_artifact,
                producer=ProducerIdentity(name="planguard", version="0.7.0", build="cli"),
            )
            if args.persist:
        ...
        if args.command == "scenario-catalog":
            store = FilesystemArtifactStore(args.store)
            catalog = build_academic_catalog(producer=ProducerIdentity(name="planguard", version="0.7.0", build="cli"))
        ...
        if args.command == "novelty-evaluate":
            ...
            novelty = evaluate_novelty(subject=manifest, bundle=bundle, corpus=corpus, producer=ProducerIdentity(name="planguard", version="0.7.0", build="cli"))
            store.save(novelty)
        ...
        if args.command == "benchmark-catalog":
            store = FilesystemArtifactStore(args.store)
            protocols = builtin_benchmark_protocols(ProducerIdentity(name="planguard", version="0.7.0", build="cli"))
            for protocol in protocols:
                store.save(protocol)
            print(canonical_json_text({"protocols": [item.model_dump(mode="json") for item in protocols]}, pretty=True))
            return 0

        if args.command == "benchmark-run":
            store = FilesystemArtifactStore(args.store)
            protocol = store.load(args.protocol_id)
            if not isinstance(protocol, BenchmarkProtocolArtifact):
                raise ValueError("protocol_id must identify a benchmark_protocol")
            producer = ProducerIdentity(name="planguard", version="0.7.0", build="cli")
            def synthetic(case, iteration, warmup):
                scale = float(next((value for value in case.values() if isinstance(value, (int, float))), 1.0))
                if args.metric_model == "constant": base = 5.0
                elif args.metric_model == "superlinear": base = max(1.0, scale ** 1.45)
                elif args.metric_model == "threshold": base = 5.0 if scale < 1000 else 500.0
                else: base = max(1.0, scale)
                jitter = (iteration % 3) * 0.01 * base
                return BenchmarkMeasurement(metrics={"wall_time_ms": base + jitter, "database_time_ms": 0.8 * base, "query_count": max(1.0, scale if args.metric_model != "constant" else 5.0)})
            series = BenchmarkRunner(producer=producer).run(protocol, synthetic)
            if args.persist: store.save(series)
            print(canonical_json_text(series, pretty=True))
            return 0

        if args.command == "security-audit":
            store = FilesystemArtifactStore(args.store)
            ids = [record.artifact_id for record in store.list()] if args.all else list(args.artifact_ids)
            artifacts = tuple(store.load(item) for item in ids)
            report = audit_artifacts(artifacts, producer=ProducerIdentity(name="planguard", version="0.7.0", build="cli"), integrity_results={item: store.verify(item) for item in ids})
            if args.persist: store.save(report)
            print(canonical_json_text(report, pretty=True))
            return 0 if str(report.payload.trust_state) == "trusted" else 2

        if args.command == "sanitize-artifact":
            store = FilesystemArtifactStore(args.store)
            artifact = store.load(args.artifact_id)
            sanitized, receipt = sanitize_artifact(artifact, producer=ProducerIdentity(name="planguard", version="0.7.0", build="cli"))
            if args.persist:
                store.save(sanitized); store.save(receipt)
            print(canonical_json_text({"sanitized": sanitized.model_dump(mode="json"), "receipt": receipt.model_dump(mode="json")}, pretty=True))
            return 0

        if args.command == "trust-verify":
            store = FilesystemArtifactStore(args.store)
            ids = [record.artifact_id for record in store.list()] if args.all else list(args.artifact_ids)
            report = verify_artifact_trust(store, ids, producer=ProducerIdentity(name="planguard", version="0.7.0", build="cli"))
            if args.persist: store.save(report)
            print(canonical_json_text(report, pretty=True))
            return 0 if str(report.payload.trust_state) == "trusted" else 2
        ...
        if args.command == "plugin-list":
            ...
            for manifest in builtin_plugin_manifests(producer):
                ...
                results.append({"manifest": manifest.model_dump(mode="json"), "loaded": True, "builtin": True})
            if args.discover:
                for result in manager.discover():
                    results.append({"manifest": result.manifest.model_dump(mode="json"), "loaded": result.loaded, "error": result.error, "builtin": False})
            if args.persist:
                store = FilesystemArtifactStore(args.store)
                for manifest in manager.manifests(): store.save(manifest)
            print(canonical_json_text({"plugins": results}, pretty=True))
            return 0

        if args.command == "demo-verify":
            store = FilesystemArtifactStore(args.store)
            cases = tuple(store.load(item) for item in args.case_ids) if args.case_ids else _all_artifacts(store, "demonstration_case")
            results = []
            failed = False
            for case in cases:
                if not isinstance(case, DemonstrationCaseArtifact): raise ValueError("case_ids must identify demonstration_case artifacts")
                valid, missing = verify_demonstration_case(case, store.load)
                failed = failed or not valid
                results.append({"case_id": case.artifact_id, "case_key": case.payload.case_key, "valid": valid, "missing": list(missing)})
            print(json.dumps({"cases": results}, indent=2))
            return 2 if failed else 0

        if args.command == "release-build":
            store = FilesystemArtifactStore(args.store)
            demos = tuple(item for item in _all_artifacts(store, "demonstration_case") if isinstance(item, DemonstrationCaseArtifact))
            plugins = tuple(item for item in _all_artifacts(store, "plugin_manifest") if isinstance(item, PluginManifestArtifact))
            audits = tuple(item for item in _all_artifacts(store, "security_audit") if isinstance(item, SecurityAuditArtifact))
            trusts = tuple(item for item in _all_artifacts(store, "artifact_trust_report") if isinstance(item, ArtifactTrustReportArtifact))
            from planguard.artifacts.models import ReleaseStatus
            release = build_release_manifest(
                release_key=args.release_key, package_version="0.7.0", demonstration_cases=demos, plugins=plugins,
                package_checksums={}, documentation_paths=("README.md", "docs/architecture.md", "docs/security.md", "docs/plugins.md"),
                validation_summary={"demonstration_cases": len(demos), "plugins": len(plugins)},
                producer=ProducerIdentity(name="planguard", version="0.7.0", build="cli"),
                security_audit=audits[-1] if audits else None, trust_report=trusts[-1] if trusts else None,
                status=ReleaseStatus(args.status),
            )
            if args.persist: store.save(release)
            print(canonical_json_text(release, pretty=True))
            return 0
