...
type Tab = "timeline" | "workload" | "families" | "findings" | "detectors" | "policies" | "manifest";
  ...
  return (
    <section>
      ...
      <div className="metric-grid four-up">
        ...
        <article className="metric-card"><span>Findings</span><strong>{data.findings.length}</strong><small>Interpretations, not graph facts</small></article>
      </div>
      ...
      <div className="tabs" role="tablist" aria-label="Run analysis views">
        {(["workload", "timeline", "families", "findings", "detectors", "policies", "manifest"] as Tab[]).map((candidate) => (
