    ...
    try:
        ...
        if graph is None:
            built = build_workload(
                ...
                producer=ProducerIdentity(name="planguard", version="0.6.0", build="workbench-api"),
...
def execute_scenario(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        if body.get("scenario_instance_id"):
            ...
            instance = instantiate(template, binding, parameters=dict(body.get("parameters", {})), variant_key=str(body.get("variant_key", "naive")), mutations=mutation_specs, seed=int(body.get("seed", 1)), producer=ProducerIdentity(name="planguard", version="0.6.0", build="workbench-api"), tags=tuple(body.get("tags", ())))
        result = ScenarioRunner(registry=catalog.registry, store=artifact_store(), producer=ProducerIdentity(name="planguard", version="0.6.0", build="workbench-api")).run(instance)
        artifact_index().sync(artifact_store())
        return JsonResponse(
            {
