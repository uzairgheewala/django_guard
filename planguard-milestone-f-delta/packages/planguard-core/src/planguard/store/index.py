...
class ArtifactIndex:
    ...
    def _metadata(artifact: Any) -> dict[str, Any]:
        ...
        if isinstance(artifact, WorkloadEpisodeArtifact):
            ...
            title = payload.title
            family_scheme_key = payload.family_scheme_key
        if isinstance(artifact, ScenarioTemplateArtifact):
            title = payload.title
            name = payload.template_key
        if isinstance(artifact, ScenarioBindingArtifact):
            title = payload.binding_key
            name = payload.application_key
        if isinstance(artifact, ScenarioInstanceArtifact):
            run_id = None
            title = f"Scenario instance {artifact.artifact_id}"
            status = "instantiated"
        if isinstance(artifact, ScenarioRunArtifact):
            run_id = payload.analysis_run_ref.artifact_id if payload.analysis_run_ref else None
            title = payload.metadata.get("template_key", "Scenario run")
            status = str(payload.status)
            mode = "laboratory"
        if isinstance(artifact, DatasetManifestArtifact):
            title = payload.dataset_key
            name = payload.scale_profile
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {"total_artifacts": total, "runs": runs, "findings": findings, "episodes": episodes, "scenarios": scenarios, "scenario_templates": templates, "plans": plans, "comparisons": comparisons, "universes": universes, "coverage_reports": coverage_reports, "counterexamples": counterexamples, "by_kind": kinds}
